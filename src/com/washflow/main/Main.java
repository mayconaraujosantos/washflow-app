    ...
    public static void main(String[] args) {
        Javalin app = Javalin.create(config -> {
            ...
            config.routes.apiBuilder(() -> {
                get("/api/health", ctx -> ctx.json(Map.of("status", "ok", "service", "washflow-api")));
                get("/api/hello", ctx -> ctx.json(Map.of("message", "Hello from Javalin + React PWA")));
            });
            ...
            config.routes.error(404, ctx -> {
                ...
                if (resource == null) {
                    ctx.contentType("text/html").result("<html><body><h1>Washflow API</h1><p>Backend is running. Frontend build not generated yet.</p></body></html>");
                    return;
                }
