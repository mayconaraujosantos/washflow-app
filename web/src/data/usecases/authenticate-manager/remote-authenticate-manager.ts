import { HttpClient, HttpStatusCode } from '@/data/protocols/http/http-client'
import { AuthenticateManager } from '@/domain/usecases/authenticate-manager'

type ErrorBody = { error?: string }

export class RemoteAuthenticateManager implements AuthenticateManager {
  constructor(
    private readonly url: string,
    private readonly httpClient: HttpClient<AuthenticateManager.Model>,
  ) {}

  async auth(
    params: AuthenticateManager.Params,
  ): Promise<AuthenticateManager.Model> {
    const httpResponse = await this.httpClient.request({
      url: this.url,
      method: 'post',
      body: params,
    })

    switch (httpResponse.statusCode) {
      case HttpStatusCode.ok:
        return httpResponse.body as AuthenticateManager.Model
      case HttpStatusCode.badRequest:
        throw new Error(
          (httpResponse.body as ErrorBody)?.error ??
            'Campo obrigatório ausente',
        )
      case HttpStatusCode.conflict:
        throw new Error(
          (httpResponse.body as ErrorBody)?.error ??
            'Este telefone já está cadastrado com outro perfil',
        )
      case HttpStatusCode.forbidden:
        // Manager login never self-registers - see
        // ManagerSelfRegistrationNotAllowedError on the backend.
        throw new Error(
          (httpResponse.body as ErrorBody)?.error ??
            'Este telefone ainda não foi promovido a gerente',
        )
      default:
        throw new Error('Erro inesperado ao entrar')
    }
  }
}
